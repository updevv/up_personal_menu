    ...
    if openmenu then
        ...
        CreateThread(function()
            while openmenu do
                ...
                if Config.Personalmenu.UniqueID then
                    RageUI.Separator(string.format("ID Unique: %s", playerData.unique_id--[[GetPlayerServerId(PlayerId())]]))
                else
                    RageUI.Separator(string.format("ID Unique: %s", GetPlayerServerId(PlayerId())))
                end
                ...
                RageUI.IsVisible(infoCrew, function()
                    local type = Config.Personalmenu.Type -- imaginons "crew"

                    RageUI.Separator(string.format("Votre Crew : ~b~%s~s~", (ESX.PlayerData[type] and ESX.PlayerData[type].label) or "undefined"))
                    RageUI.Separator(string.format("Votre Rang : ~b~%s~s~", (ESX.PlayerData[type] and ESX.PlayerData[type].grade_label) or "undefined"))
                    RageUI.Line()
                    local isNoCrew = ESX.PlayerData[type].name ~= Config.Personalmenu.CrewName
